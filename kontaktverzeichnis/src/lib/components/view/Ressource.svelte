  ...
  <Tile light>
    {#if edit}
      <form class="top-right-button" action="?/save" method="POST" use:enhance>
        <Button on:click={resetForm} type="submit" value="{JSON.stringify(data.ressource)}" name="data" icon={AcceptIcon} size="small" kind="ghost" iconDescription="Änderungen speichern"></Button>
      </form>
    {/if}
